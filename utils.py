...
def cosine_similarity_matrix(gene_emb, peak_emb):
    """
    计算 gene embedding 和 peak embedding 的两两余弦相似度矩阵

    Parameters
    ----------
    gene_emb : torch.Tensor
        shape = [num_genes, dim]
    peak_emb : torch.Tensor
        shape = [num_peaks, dim]

    Returns
    -------
    sim_matrix : torch.Tensor
        shape = [num_genes, num_peaks]
        sim_matrix[i, j] 表示第 i 个 gene 和第 j 个 peak 的余弦相似度
    """
    ...
    peak_emb = F.normalize(peak_emb, p=2, dim=1)

    # 矩阵乘法得到两两余弦相似度
    sim_matrix = torch.matmul(gene_emb, peak_emb.T)
