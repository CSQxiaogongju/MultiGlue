...
def construct_neighbor_graph(adata_omics1, adata_omics2, datatype='SPOTS', n_neighbors=3): 
    ...
    adata_omics1.uns['adj_spatial'] = adj_omics1 # original adjacent matrix
    ...
    adata_omics2.uns['adj_spatial'] = adj_omics2 # original adjacent matrix
...
def adjacent_matrix_preprocessing(adata_omics1, adata_omics2):
    ...
    # Neighbor graph (dense)
    graph_neighbor = adj_spatial_omics1.toarray()
...
def neighbor_context_features(adata, mode: str = "mean"):
    """
    A: (N, N) 0/1 adjacency, A[i,j]=1 means j is neighbor of i
    X: (N, F) node/spot features
    mode: "sum" or "mean"
    return: (N, F) contextual features aggregated from neighbors only (exclude self)
    """
    #print(adata)
    key_feat = 'feat'
    if key_feat in adata.obsm:
       X = np.asarray(adata.obsm[key_feat]) 
    else:
       raise ValueError(f"{key_feat} does not exist in adata.obsm")
       
    key_adj = 'adj_spatial' 
    if key_adj in adata.uns:
       A = transform_adjacent_matrix(adata.uns[key_adj]) 
       A = sp.coo_matrix(A) 
    else:
       raise ValueError(f"{key_adj} does not exist in adata.obsm") 

    if sp.issparse(A):  # removing self-loop
       A = A.tocsr(copy=True)
       A.setdiag(0)
       A.eliminate_zeros()
    ...
    if mode == "sum":
        ...
        deg_safe = np.maximum(deg, 1.0)          # avoid divide-by-zero for isolated nodes
        return X_neigh / deg_safe[:, None]   
